		...
		case string:
			...
			case ResourceTypeSchemasKey:
				...
				if err := dec.Decode(&acceptValue); err != nil {
					return fmt.Errorf(`failed to decode vlaue for %q: %w`, ResourceTypeSchemasKey, err)
				}
				var val schemas
				err = val.AcceptValue(acceptValue)
				if err != nil {
					return fmt.Errorf(`failed to accept value for %q: %w`, ResourceTypeSchemasKey, err)
				}
				v.schemas = &val
			default:
				var val interface{}
